...
def evaluate_quality(input_dir: Path, staleness_days: int = 30) -> dict[str, Any]:
    ...
    for profile in csv_profiles:
        ...
        if not profile["valid_width"]:
            anomalies.append(_anomaly(source_key, "DQ06", "blocking", "Invalid file width", "quarantine"))
        if profile["section_state"] == "empty":
            anomalies.append(_anomaly(source_key, "DQ23", "info", "Section present but empty", "review"))
    ...
    for case in cases:
        ...
        if kind == "invalid_duration":
            anomalies.append(_anomaly(case_id, "DQ12", "warning", "Duration cannot be converted", "quarantine"))
        elif kind == "stale_price":
            age = (date.fromisoformat(case["business_date"]) - date.fromisoformat(case["price_date"])).days
            if age > staleness_days:
                anomalies.append(_anomaly(case_id, "DQ15", "warning", "Stale price", "review"))
        elif kind == "nav" and Decimal(str(case["value"])) <= 0:
            anomalies.append(_anomaly(case_id, "DQ07", "blocking", "Non-positive NAV", "quarantine"))
        elif kind == "drawdown" and not -1 <= Decimal(str(case["value"])) <= 0:
            anomalies.append(_anomaly(case_id, "DQ24", "blocking", "Drawdown outside valid domain", "quarantine"))
        elif kind == "forbidden_name":
            anomalies.append(_anomaly(case_id, "DQ20", "blocking", "Forbidden name detected in input", "quarantine"))
        elif kind == "private_identifier":
            anomalies.append(_anomaly(case_id, "DQ21", "blocking", "Private identifier detected in input", "quarantine"))
        elif kind == "peer_identifier":
            peer_id = case["value"]
            if peer_id in peer_seen:
                anomalies.append(_anomaly(case_id, "DQ22", "warning", "Duplicate synthetic peer", "quarantine"))
            peer_seen.add(peer_id)
        elif kind == "empty_section":
            anomalies.append(_anomaly(case_id, "DQ23", "info", "Section present but empty", "review"))
        elif kind == "uncertain_bridge":
            anomalies.append(_anomaly(case_id, "DQ30", "warning", "Fund-of-funds relationship pending validation", "review"))
